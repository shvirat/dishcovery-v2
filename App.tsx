...
import { 
  Search, Heart, User as UserIcon, LogOut, Sun, Moon, 
  Menu, X, ChefHat, Sparkles, Wand2, Compass, History, Info
} from 'lucide-react';
...
const Navbar: React.FC<{ 
  ...
  if (auth.isAuthenticated) {
    navLinks.push({ name: 'Favorites', path: '/favorites', icon: <Heart size={18} /> });
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">Dishcovery</span>
          </Link>
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-orange-500 ${
                    location.pathname === link.path ? 'text-orange-500' : 'text-slate-600 dark:text-slate-400'
                  }`}
                >
                  {link.icon}
                  {link.name}
...
export default function App() {
  ...
  return (
    <BrowserRouter>
      <div className={`min-h-screen transition-colors ${theme === Theme.DARK ? 'dark' : ''} bg-slate-50 dark:bg-slate-950`}>
        <Navbar theme={theme} toggleTheme={toggleTheme} auth={auth} onLogout={handleLogout} />
        
        <main className="pb-12">
          <Routes>
            <Route path="/" element={<HomePage />} />
            <Route path="/meal/:id" element={<MealDetailsPage auth={auth} setAuth={handleAuthChange} />} />
            <Route path="/favorites" element={<FavoritesPage auth={auth} setAuth={handleAuthChange} />} />
            <Route path="/login" element={<LoginPage auth={auth} onLogin={handleAuthChange} />} />
            <Route path="/signup" element={<SignupPage auth={auth} onLogin={handleAuthChange} />} />
            <Route path="/ai-lab" element={<AIMealLabPage auth={auth} setAuth={handleAuthChange} />} />
        ...
        <footer className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 py-12">
          <div className="max-w-7xl mx-auto px-4 text-center">
            <div className="flex justify-center items-center gap-2 mb-4">
              ...
              <span className="text-lg font-bold text-slate-900 dark:text-white">Dishcovery</span>
            </div>
            <p className="text-slate-500 dark:text-slate-400 text-sm max-w-md mx-auto">
              The ultimate AI-powered culinary companion. Discover, save, and visualize your next favorite meal.
            </p>
            <div className="mt-8 flex justify-center gap-6">
              <a href="/terms" className="text-slate-400 hover:text-orange-500 transition-colors text-sm">Terms</a>
              <a href="/privacy" className="text-slate-400 hover:text-orange-500 transition-colors text-sm">Privacy</a>
              <a href="/about" className="text-slate-400 hover:text-orange-500 transition-colors text-sm">About</a>
              <a href="/contact" className="text-slate-400 hover:text-orange-500 transition-colors text-sm">Contact</a>
            </div>
            <div className="mt-8 text-slate-500 dark:text-slate-600 text-xs">
                © {new Date().getFullYear()} Dishcovery. Built with human creativity.
            </div>
          </div>
        </footer>
      </div>
